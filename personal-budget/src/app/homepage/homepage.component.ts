...
export class HomepageComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.http.get('http://localhost:3000/budget').subscribe((res: any) => {
      ...
      for (var i = 0; i < res.Aishwarya_Budget.length; i++) {
        this.dataSource.datasets[0].data[i] = res.Aishwarya_Budget[i].budget;
        this.dataSource.labels[i] = res.Aishwarya_Budget[i].title;
      }
      this.createChart();
  ...
  createChart() {
    ...
    createD3JSChart(data: any[]) {
      ...
      const color = d3.scaleOrdinal<string, string>()
        ...
        .range(['#ff4c4c', '#ff6666', '#ff7f7f', '#ff9999', '#ffb2b2', '#ffcccc', '#ffe5e5']);
